...
void Comp::merge(vector<int> &v, int l, int m, int r) {
...
void Comp::ms(vector<int> &v, int l, int r) {
  if (l < r && l >= 0 && r < v.size()) {
    int m = (r - l) / 2 + l;
...
void Comp::createAccount(string id, int count) {
  ...
  if (bankStorage1d[idx].id == "") {bankStorage1d[idx] = acc1;} else {
    long long int j = 1;
    for (long long int i = idx; i < bankStorage1d.size(); i = (i + j * j) % 200003) {
...
{
  long long int  myhashval = 0;
  long long int o = 1;
  long long int p = 9973 ;
  for(auto c : id)
  {
    myhashval = (myhashval + (o*(c+1)))%200003 ;
    o = (o * p ) % 200003;
  }
  return myhashval;
}